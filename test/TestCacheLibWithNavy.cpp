...
void initCache() {
    ...
    navyConfig.setSimpleFile(cacheDir_ + "/navy", 1024 * 1024ULL * 1024ULL);
    navyConfig.setDeviceMetadataSize(16 * 1024 * 1024);
    // navyConfig.setBlockSize(1024);
    navyConfig.setNavyReqOrderingShards(10);

    // Region is 1MB
    navyConfig.blockCache().setRegionSize(1 * 1024 * 1024);
    ...
    // set overall cache config
    {
        allocConfig_.enableCachePersistence(cacheDir_);
        // Set the in-memory cache size to 1GB
        allocConfig_.setCacheSize(1024 * 1024 * 1024);
